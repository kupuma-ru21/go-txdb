...
var UserRels = struct {
	...
	UserIDLinks: "UserIDLinks",
}

// userR is where relationships are stored.
type userR struct {
	UserIDLinks LinkSlice `boil:"UserIDLinks" json:"UserIDLinks" toml:"UserIDLinks" yaml:"UserIDLinks"`
}
...
func (r *userR) GetUserIDLinks() LinkSlice {
	if r == nil {
		return nil
	}
	return r.UserIDLinks
}
...
// UserIDLinks retrieves all the Link's Links with an executor via UserID column.
func (o *User) UserIDLinks(mods ...qm.QueryMod) linkQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`Links`.`UserID`=?", o.ID),
	)

	return Links(queryMods...)
}

// LoadUserIDLinks allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUserIDLinks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`Links`),
		qm.WhereIn(`Links.UserID in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.UserIDLinks = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.UserID) {
				local.R.UserIDLinks = append(local.R.UserIDLinks, foreign)
				if foreign.R == nil {
					foreign.R = &linkR{}
				}
...
// AddUserIDLinks adds the given related objects to the existing relationships
// of the User, optionally inserting them as new records.
// Appends related to o.R.UserIDLinks.
// Sets related.R.UserIDUser appropriately.
func (o *User) AddUserIDLinks(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Link) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.UserIDLinks = append(o.R.UserIDLinks, related...)
	}
...
// SetUserIDLinks removes all previously related items of the
// User replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.UserIDUser's UserIDLinks accordingly.
// Replaces o.R.UserIDLinks with related.
// Sets related.R.UserIDUser's UserIDLinks accordingly.
func (o *User) SetUserIDLinks(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Link) error {
	query := "update `Links` set `UserID` = null where `UserID` = ?"
	...
	if o.R != nil {
		for _, rel := range o.R.UserIDLinks {
	...
	return o.AddUserIDLinks(ctx, exec, insert, related...)
}

// RemoveUserIDLinks relationships from objects passed in.
// Removes related items from R.UserIDLinks (uses pointer comparison, removal does not keep order)
// Sets related.R.UserIDUser.
func (o *User) RemoveUserIDLinks(ctx context.Context, exec boil.ContextExecutor, related ...*Link) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.UserIDLinks {
			if rel != ri {
				continue
			}

			ln := len(o.R.UserIDLinks)
			if ln > 1 && i < ln-1 {
				o.R.UserIDLinks[i] = o.R.UserIDLinks[ln-1]
			}
			o.R.UserIDLinks = o.R.UserIDLinks[:ln-1]
			break
		}
	}
